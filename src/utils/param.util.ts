...
export class ParamUtil {
     ...
     * @returns 相同的短信模板id
     */
    async findSameTemplateId(template: SmsTemplate[]): Promise<string[]> {
        const existTemplateId = [];
        const templateIds = template.map(item => item.templateId);
        templateIds.forEach(item => {
            (templateIds.indexOf(item) !== templateIds.lastIndexOf(item) && existTemplateId.indexOf(item) === -1) && existTemplateId.push(item);
        });
